<?php

/**
 * Seznam všech členů týmu
 */
return [

    /**
     * Patrik
     */
    [
        "name" => "Patrik Půlpán",
        "nickname" => "Paťa",
        "town" => "Slatiňany",
        "img" => "Pata_web.jpg",

        "likes" => "Skauting, Organizace akcí, divadlo",
        "meet" => "Všude mě je všude plno a hlavně jsem slyšet přes celé tábořiště :)",
        "about" => "Jsem skaut už 13 let a pořád mě to baví! Jsem vůdce jedné smečky vlčat, motám se kolem vedení jednoho střediska, podílím se realizaci dalších kurzů :) a ještě pár věcí ... Mimo to jsem student a ještě nevím čím bych chtěl v životě být (ale popelářem už ne!)",
        "education" => [
            "ČK Tokaheya (2014)",
            "ZZA Podlitina (2015)",
            "VLK Quo Vadis (2015)"
        ],
        "take" => "Špunty do uší, dobrou náladu a hlavně peřinky!",
        "motto" => "Kdo chce jiné zapalovat, musí sám hořet!"
    ],

    /**
     * Tereza
     */
    [
        "name" => "Tereza Koberová",
        "nickname" => "Sňéža",
        "town" => "Slatiňany",
        "img" => "tereza.jpg",

        "likes" => "Trempování",
        "meet" => "49° 53' 18.0901577\" N 15° 46' 10.357132\" E",
        "about" => "Mé jméno už znáte a studuji medicinální chemii (takže si s sebou raději vemte ochutnávače). Skautuju ve Slatiňanech od té doby, co jsem přišla na světlušácký nábor. Prázdniny ráda trávím v přírodě, baví mě objevovat nová místa - v zimě na běžkách, v létě pěšmo.",
        "education" => [
            "2014 Podlitina", "2015 ČLK Odyssea", "2017 VLK Velká Morava",
        ],
        "take" => "To, co mi při balení cvrnkne o nos.",
        "motto" => "Žij naplno (skoro) každý den!"
    ],

    /**
     * Ondra
     */
    [
        "name" => "Ondřej Procházka",
        "nickname" => "MR. OP",
        "town" => "Sobětuchy",
        "img" => "ondra.png",

        "likes" => "",
        "meet" => "Tam kde mě budeš hledat",
        "about" => "Nic ti neřeknu!",
        "education" => [
        "to nepotřebuju abych tě mohl učit"
        ],
        "take" => "počítač, GoPro, a dávku srandy",
        "motto" => "Kdo nic nedělá nic nezkazí."
    ],

    /**
     * Přemek
     */
    [
        "name" => "Přemysl Janda",
        "nickname" => "Přemek",
        "town" => "Slatiňany",
        "img" => "ondra.png",

        "likes" => "",
        "meet" => "To se ještě uvidí, ale jisté je že žádnou rozcvičku nevynechám.",
        "about" => "Druhým rokem studuji vysokou zednickou na ČVUT v Praze. Už nějaký ten pátek skautuju v 1. oddíle Dúnadan ve Slatiňanech. Občas si rád zahraju frisbee, mám radši slané, ale sladkým nikdy nepohrdnu. ",
        "education" => [
            "2014 ČK Zvostra",
            "2016 ZZA Cukříček", 
        ],
        "take" => "Nikdy nesmí chybět polštář.Dobrý spánek vždy osladí život.",
        "motto" => "Vždycky se to nějak udělá."
    ],

    /**
     * Šimon
     */
    [
        "name" => "Šimon Kimák",
        "nickname" => "Krocan",
        "town" => "Slatiňany",
        "img" => "ondra.png",

        "likes" => "Skauting, příroda, literatura frisbee, běžky.",
        "meet" => "na přednáškách a občas zaskočím do kurzovní kuchyně (ikdyž teď už jenom ze zvědavosti)",
        "about" => "Jsem Šíma a studuji na gymplu, mám moc rád přírodu a taky mě baví chemie. Rád bych se v budoucnu věnovat studiu přírodních věd, či medicíny.
                    Skautem jsem od první třídy. Už čtyři a půl roku rádcuji vlčata, nyní jsem zástupce vůdce oddílu. 
                    mezi mé záliby patří fantazi literatura (např: Úžasná Zeměplocha), rád vařím a chodím na dlouhé procházky a nebo jezdím na kole.",
        "education" => [
        "2015 ČLK Corda", "2015 ZZA Quo Vadis", "2015 VLK Gong",
        ],
        "take" => "dobrou knížku, křesadlo a veselý úsměv",
        "motto" => "Mám taková dvě životní motta, jedno je takové které mě celkem vystihuje a to druhá je takové krásně hlubokomyslné. 1.Zatopíš, uvaříš, pomůžeš, vysvětlíš.  To druhé motto je citát z mé oblíbené knihy: ,,Rozhodovat můžeme jen o tom, co udělat s časem, který nám byl dán... (Gandalf) Společenstvo prstenu"
    ],

    /**
     * Sylva
     */
    [
        "name" => "Sylva Coufalová",
        "nickname" => "Syslík",
        "town" => "Kroměříž",
        "img" => "Sylva_web.jpg",

        "likes" => "",
        "meet" => "Na kurzu budu hlavně vařit a usmívat se na vás, možná pomůžu s nějakou přednáškou:)",
        "about" => "Ahoj, je mi 16 a skautka jsem od svých 8 let. Chodím na gymnázium. Mám ráda skauting, jídlo, internet, facebook, instagram, postel, biologii, zsv, vaření, malování a chození na procházky:)",
        "education" => [
        "2015 RK Cukříček",
        ],
        "take" => "Určitě si vezmu polštář a rukavice, protože u ohně je někdy zima. Jídlo rozhodně ne, protože bych pak neměla místo na to skvělý, co na nás čeká na kurzu! <3",
        "motto" => "\"Jez dokud můžeš, protože nikdy nevíš, kdy budeš jíst příště!:)\""
    ],

    /**
     * Mája
     */
    [
        "name" => "Magdaléna Vtípilová",
        "nickname" => "Mája",
        "town" => "Kroměříž",
        "img" => "Maja_web.jpg",

        "likes" => "",
        "meet" => "Hlavně v kuchyni, občas na nějakém programu a možná i na malé přednášce. ",
        "about" => "Ahoj všichni! Jsem Mája a svůj volný čas nejraději trávím u nějakého hudebního nástroje. Do našeho oddílu na Dobříši  chodím teď sedmým rokem a momentálně vedu družinku skautek.:) Mám také ráda pečení dobrých věcí a organizaci různých akcí s dětmi. Jen tak mimochodem jsem se Sylvou absolventka 1. ročníku!",
        "education" => [
        "2015 RK Cukříček",
        ],
        "take" => "Kytaru, zpěvník a vařečku. ",
        "motto" => "\"Jediná nemožná cesta je ta, na kterou se nikdy nevydáte.\""
    ],

    /**
     * Sylva
     */
    [
        "name" => "Lucie Procházková",
        "nickname" => "Lucka",
        "town" => "Sobětuchy",
        "img" => "patrik.png",

        "likes" => "",
        "meet" => "",
        "about" => "",
        "education" => [
        "2015 RK Cukříček",
        ],
        "take" => "",
        "motto" => ""
    ],
];